...
export async function changeEstado(id_orden, id_estado_orden, nombre_estado, current_orden) {
    ...
    try {
        ...
        if (id_estado_orden === 4) {
            // Entregada
            await db.run("UPDATE orden_trabajo SET id_estado_orden = ?, fecha_entrega_real = datetime('now','localtime') WHERE id_orden = ?", [id_estado_orden, id_orden], false);
            // ALSO update all items (prendas) to 'Entregada' (id_estado_prenda = 4)
            await db.run("UPDATE prenda SET id_estado_prenda = 4 WHERE id_orden = ?", [id_orden], false);
        } else {
            await db.run("UPDATE orden_trabajo SET id_estado_orden = ? WHERE id_orden = ?", [id_estado_orden, id_orden], false);
        }
