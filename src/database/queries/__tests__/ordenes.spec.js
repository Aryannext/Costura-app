...
describe("Ordenes Queries Transactions", () => {
  ...
  describe("createOrden", () => {
    it("should successfully use explicit transaction and return run lastId", async () => {
      ...
      const resultId = await createOrden({
        fecha_entrega_estimada: "2023-12-01",
        id_cliente: 5,
      });
      ...
      expect(calledSet[0].statement).toContain(
      ...
      // Check that saveDb is called after commit
      const { saveDb } = await import("../../connection.js");
    ...
    it("should rollback and bubble up error if run fails", async () => {
      ...
      await expect(
        createOrden({ fecha_entrega_estimada: "2023-12-01", id_cliente: 5 }),
      ).rejects.toThrow("DB Error on run");
    ...
    it("should rollback and bubble up error if executeSet fails", async () => {
      ...
      await expect(
        createOrden({ fecha_entrega_estimada: "2023-12-01", id_cliente: 5 }),
      ).rejects.toThrow("DB Error on executeSet");
    ...
    it("should NOT rollback if saveDb fails (transaction already committed), and bubble up error", async () => {
      const fakeError = new Error("IndexedDB save failure");
      db.beginTransaction.mockResolvedValueOnce();
      db.run.mockResolvedValueOnce({ changes: { lastId: 30 } });
      db.executeSet.mockResolvedValueOnce({});
      db.commitTransaction.mockResolvedValueOnce();

      const { saveDb } = await import("../../connection.js");
      saveDb.mockRejectedValueOnce(fakeError);

      await expect(
        createOrden({ fecha_entrega_estimada: "2023-12-01", id_cliente: 5 }),
      ).rejects.toThrow("IndexedDB save failure");
  ...
  describe("changeEstado", () => {
    it("should executeSet with 2 queries for normal state change", async () => {
      ...
      expect(calledSet[1].statement).toContain(
        "INSERT INTO historial_actividad",
      );
    });
    ...
    it("should executeSet with 3 queries for Entregada state change (updates prendas)", async () => {
      ...
      expect(calledSet[0].statement).toContain("UPDATE orden_trabajo");
      expect(calledSet[1].statement).toContain(
        "UPDATE prenda SET id_estado_prenda = 4",
      );
      expect(calledSet[2].statement).toContain(
        "INSERT INTO historial_actividad",
      );
    });
    ...
    it("should bubble up error and rollback entirely if executeSet fails", async () => {
      ...
      await expect(changeEstado(1, 2, "Procesando", null)).rejects.toThrow(
        "DB Error",
      );
      expect(db.executeSet).toHaveBeenCalledTimes(1);
    });
